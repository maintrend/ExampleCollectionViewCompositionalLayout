...
class AdvancedViewController: UIViewController {
    ...
    func setupCollectionView() {
        ...
        collectionView.register(BigCell.self, forCellWithReuseIdentifier: BigCell.reuseId)
    ...
    private func setupDataSource() {
        dataSource = UICollectionViewDiffableDataSource<SectionKind, Int>(collectionView: collectionView, cellProvider: { (collectionView, indexPath, intValue) -> UICollectionViewCell? in
            ...
            case .gridBig:
                return self.configure(cellType: BigCell.self, with: intValue, for: indexPath)
            }
        })
    }
    ...
    private func createLayout() -> UICollectionViewLayout {
        let layout = UICollectionViewCompositionalLayout { (sectionIndex, layoutEnvirnment) -> NSCollectionLayoutSection? in
            ...
            case .gridBig:
                return self.createBigGridSection()
            }
        }
    ...
    private func createBigGridSection() -> NSCollectionLayoutSection {
        let itemSize = NSCollectionLayoutSize(widthDimension: .fractionalWidth(1),
                                              heightDimension: .fractionalHeight(1))
        let layoutItem = NSCollectionLayoutItem(layoutSize: itemSize)
        layoutItem.contentInsets = NSDirectionalEdgeInsets.init(top: 0, leading: 8, bottom: 0, trailing: 8)
        ...
        let layoutGroupSize = NSCollectionLayoutSize(widthDimension: .estimated(240),
